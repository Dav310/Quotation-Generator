    ...
    <div className="mb-8 relative group">
      <SectionHeader title="8) ASSUMPTIONS & EXCLUSIONS">
        <div className="print:hidden flex gap-2">
          <button
            ...
            className="bg-white/20 hover:bg-white/40 text-white px-2 py-0.5 rounded text-[10px] transition-all"
          >
            + Add Assumption
          </button>
          <button
      ...
      <div className="border-x border-b border-blue-200 p-4 bg-yellow-50/30">
        <ul className="list-disc pl-5 space-y-2">
          ...
          {localData.exclusions.map((item, idx) => (
            <li key={`excl-${idx}`} className="text-gray-700 relative group/row flex items-center pr-4">
              <div className="flex-1">
                <SeamlessInput
                  value={item}
                  onChange={(v: string) => {
                    const newArr = [...localData.exclusions];
                    newArr[idx] = v;
                    setLocalData({ ...localData, exclusions: newArr });
                  }}
                />
              </div>
              <RemoveButton onClick={() => removeArrayItem("exclusions", idx)} className="right-0" />
            </li>
          ))}
        </ul>
      </div>
